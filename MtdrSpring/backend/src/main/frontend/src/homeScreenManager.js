...
function TaskItem({ task, onClick }) {
  return (
    <div className={`task-item ${task.priority.toLowerCase()}`} onClick={onClick}>
      ...
      <p>Usuario: {task.user.split('@')[0]}</p>
    </div>
  );
}

// Componente principal de la pantalla de inicio para el manager
function HomeScreenManager({ username }) {
    ...
    const fetchAllTasks = async () => {
      try {
        ...
        for (const userDoc of usersSnapshot.docs) {
          ...
          if (userRole !== 'manager' && !emails.includes(userEmail)) {
            emails.push(userEmail);

            const userTasksSnapshot = await getDocs(collection(firestore, 'tasks', userEmail, 'user_tasks'));
            const userTasks = userTasksSnapshot.docs.map(taskDoc => ({ id: taskDoc.id, user: userEmail, ...taskDoc.data() }));
  ...
  const filteredTasks = selectedUser === 'Todos' ? tasks : tasks.filter(task => task.user === selectedUser);
  const filterText = selectedUser === 'Todos' ? 'Tareas de Todos los Usuarios:' : `Tareas de ${selectedUser.split('@')[0]}:`;
  ...
  return (
    <div className={`home-screen ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <h1 className="welcome-text">
        Bienvenido a la página de inicio para MANAGER <span role="img" aria-label="manager">👨‍💼</span>
      </h1>
      ...
      <div className="filter-container">
        ...
        <select id="user-filter" value={selectedUser} onChange={e => setSelectedUser(e.target.value)}>
          ...
          {userEmails.filter(email => email !== 'realmanager@gmail.com').map(email => (
